    <article>
        <div class="main_tittle_box">
            <div class="main_tittle_mini">
                <h3 class="pink_tittle">REVIEWS</h3>
            </div>
        </div>
    </article>

    <div class="reg_wrap">
        <form class="reg_block_left" action="index.php?c=reviews&status=success" method="POST" enctype="multipart/form-data">
            <p class="reg_tittle_name">Your Name</p>
            <input class="reg_inp" name="name_rev" type="text" required placeholder="Name">
            <br>
            <p class="reg_tittle_name">Your city</p>
            <input class="reg_inp" name="city_rev" type="text" required placeholder="City">
            <br>
            <p class="reg_tittle_name">Your photo</p>
            <input class="reg_inp" type="file" name="photo_rev" accept="image/*">
            <textarea class="reg_inp" type="text" name="text_review" required placeholder="Your review about us" rows="20"></textarea>
            <p class="instr">Please use 8 or more characters, with at least 1 number and a mixture of uppercase and
                lowercase letters</p>
            <?php if ($_GET['c'] == 'reviews') {
                $reviews->add_capture(); ///РЕНДЕРИНГ КАПЧИ
            } ?>
            <button class="reg_gend_btn" type="submit">
                JOIN NOW
        ...
        <div class="reviews_box">
            <?php
            if ($_GET['c'] == 'reviews') {
                $reviews->render(); ///РЕНДЕРИНГ СПИСКА ОТЗЫВОВ
            }
            ?>
        </div>
    </div>